...
function RouteComponent() {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-300 flex flex-col justify-center items-center relative">
            ...
            <div onClick={() => { setShowModal(false) }} onKeyPress={() => { setShowModal(false) }} className={twMerge(clsx("absolute h-full w-full opacity-30 bg-slate-300 z-10 invisible", {
                visible: showModal
            }
            ))} />

            {/* a modal that lets users copy paste their custom wordle link */}
            <div className={twMerge(clsx("absolute h-[150px] w-[400px] bg-slate-800 rounded-2xl flex flex-col justify-center items-center z-20 text-slate-400 font-bold text-lg not-sm:w-[350px] not-sm:text-[15px] invisible",
                {
                    visible: showModal
                }
            ))}>
                <p className="sm:text-lg mb-auto mt-4">Your custom wordle link has been created!</p>

                <div className="flex justify-center items-center gap-4 mb-8">
                    <input type="text" readOnly value={`${document.location.origin}/custom/${wordleId}`} className="outline-none border-2 border-slate-500 rounded-lg p-2 focus:border-slate-300" />
                    <button type="button" className="p-2 bg-slate-400 rounded-lg hover:cursor-pointer hover:bg-slate-500 active:bg-slate-600" onClick={async () => {
                        await navigator.clipboard.writeText(`${document.location.origin}/custom/${wordleId}`)
                    }}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-slate-950">
            ...
            <p className={twMerge(clsx("mt-4 text-lg text-red-500 invisible", {
                "visible": showErrorMessage
            }))}>Something went wrong.<span><br /></span> Please try again later.</p>
        </div>
    );
}
